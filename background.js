chrome.contextMenus.create({
    ...
    onclick: function (info, tab) {
        ...
        var i = -1;
        var day = 1;
        if ((i = t.search(/8\/14|14日|金曜|[1一]日目|金\W*[東西A-Zア-ロあ-れ]|初日/)) >= 0) day = 1;
        else if ((i = t.search(/8\/15|15日|土曜|[2二]日目|土\W*[東西A-Zア-ロあ-れ]/)) >= 0) day = 2;
        else if ((i = t.search(/8\/16|16日|日曜|[3三]日目|日\W*[東西A-Zア-ロあ-れ]|最終日/)) >= 0) day = 3;

        if (i > 0) { // 日付・曜日以降の値を使用
            t = t.substr(i);
        }

        if (!t.match(/(?:C88)?.*([A-Zア-ロあ-れ])\W*([0-9]{1,2})\W*([ab])?/)) {
