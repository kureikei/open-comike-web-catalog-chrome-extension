chrome.contextMenus.create({
    ...
    onclick: function (info, tab) {
        ...
        else if ((i = t.search(/8\/16|16日|土曜|[2二]日目|土\W*[東西A-Zア-ロあ-れ]/)) >= 0) day = 2;
        else if ((i = t.search(/8\/17|17日|日曜|[3三]日目|日\W*[東西A-Zア-ロあ-れ]|最終日/)) >= 0) day = 3;

        if (i > 0) { // 日付・曜日以降の値を使用
            t = t.substr(i);
        }

        if (!t.match(/(?:C86)?.*([A-Zア-ロあ-れ])\W*([0-9]{1,2})\W*([ab])?/)) {
